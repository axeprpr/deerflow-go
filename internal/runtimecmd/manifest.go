...
func (m NodeManifest) StartupLines() []string {
	return []string{
		...
		fmt.Sprintf("  sandbox_profile=isolation=%s capabilities=%s heartbeat_ms=%d idle_ttl_ms=%d max_active_leases=%d", firstNonEmpty(m.SandboxProfile.Isolation, "(unknown)"), strings.Join(m.SandboxProfile.Capabilities, ","), m.SandboxProfile.Limits.HeartbeatIntervalMilli, m.SandboxProfile.Limits.IdleTTLMilli, m.SandboxProfile.Limits.MaxActiveLeases),
