...
func (s *Server) registerLangGraphRoutes(mux *http.ServeMux, prefix string) {
	...
	mux.HandleFunc("GET "+prefix+"/threads/{thread_id}/memory-scope", s.handleThreadMemoryScopeGet)
	mux.HandleFunc("PUT "+prefix+"/threads/{thread_id}/memory-scope", s.handleThreadMemoryScopePut)
	mux.HandleFunc("DELETE "+prefix+"/threads/{thread_id}/memory-scope", s.handleThreadMemoryScopeDelete)
	mux.HandleFunc("GET "+prefix+"/threads/{thread_id}/memory", s.handleThreadMemoryGet)
	mux.HandleFunc("DELETE "+prefix+"/threads/{thread_id}/memory", s.handleThreadMemoryClear)
	mux.HandleFunc("GET "+prefix+"/threads/{thread_id}/memory/export", s.handleThreadMemoryExport)
	mux.HandleFunc("POST "+prefix+"/threads/{thread_id}/memory/import", s.handleThreadMemoryImport)
	mux.HandleFunc("POST "+prefix+"/threads/{thread_id}/memory/reload", s.handleThreadMemoryReload)
	mux.HandleFunc("POST "+prefix+"/threads/{thread_id}/memory/facts", s.handleThreadMemoryFactCreate)
	mux.HandleFunc("PATCH "+prefix+"/threads/{thread_id}/memory/facts/{fact_id}", s.handleThreadMemoryFactUpdate)
	mux.HandleFunc("DELETE "+prefix+"/threads/{thread_id}/memory/facts/{fact_id}", s.handleThreadMemoryFactDelete)
